#PROJECTPATH>C:/Users/VIRTUALENVPATH/Scripts/python.exe D:/PYINSTALLERPATH/pyinstaller.py --onefile --noconsole core.py

import Tkinter as tk
from PIL import ImageTk # If it is ImageTk, pyinstaller doesn't work
import tkFont
...
OPTION_LABELS = ["Leaf", "Leaf Type", "Leaf Shape", "Leaf base shape", "Leaf tip shape", "Leaf margin", "Leaf venation",
                 "Species", "Contributor"]
OPTION_NAMES = [["YES", "NO"], ["SIMPLE", "COMPOUND"],
                ["ACEROSE", "AWL-SHAPED", "GLADIATE", "HASTATE", "CORDATE", "DELTOID", "LANCEOLATE", "LINEAR",
                 "ELLIPTIC", "ENSIFORM", "LYRATE",
                 "OBCORDATE", "FALCATE", "FLABELLATE", "OBDELTOID", "OBELLIPTIC", "OBLANCEOLATE", "OBLONG",
                 "PERFOLIATE", "QUADRATE", "OBOVATE", "ORBICULAR",
                 "RENIFORM", "RHOMBIC", "OVAL", "OVATE", "ROTUND", "SAGITTATE", "PANDURATE", "PELTATE", "SPATULATE",
                 "SUBULATE"],
                ["AEQUILATERAL", "ATTENUATE", "AURICULATE", "CORDATE", "CUNEATE", "HASTATE", "OBLIQUE", "ROUNDED",
                 "SAGITTATE", "TRUNCATE"],
                ["CIRROSE", "CUSPIDATE", "ACUMINATE", "ACUTE", "EMARGINATE", "MUCRONATE", "APICULATE", "ARISTATE",
                 "MUCRONULATE", "MUTICOUS", "ARISTULATE", "CAUDATE", "OBCORDATE", "OBTUSE", "RETUSE", "ROUNDED",
                 "SUBACUTE", "TRUNCATE"]
    , ["BIDENTATE", "BIFID", "DENTATE", "DENTICULATE", "BIPINNATIFID", "BISERRATE", "DIGITATE", "DISSECTED", "CLEFT",
       "CRENATE", "DIVIDED", "ENTIRE", "CRENULATE", "CRISPED", "EROSE", "INCISED", "INVOLUTE", "LACERATE", "PEDATE",
       "PINNATIFID", "LACINIATE", "LOBED", "PINNATILOBATE", "PINNATISECT", "LOBULATE", "PALMATIFID", "REPAND",
       "REVOLUTE", "PALMATISECT", "PARTED", "RUNCINATE", "SERRATE", "SERRULATE", "SINUATE", "TRIDENTATE", "TRIFID",
       "TRIPARTITE", "TRIPINNATIFID"], ["RETICULATE", "PARALLEL"], ["SPECIES FIELD"], ["CONTRIBUTOR FIELD"]]
...
button_images = []
button_image_paths = ["open", "center", "zoomin", "zoomout", "qm", "label"]
...
def show_image(f):
    ...
    if open_once == False or current_image == None or not resize_once:
        try:
            ...
            current_image_resized = current_image.resize(
                (int(round(optimal_zoom_w * ww / (0.5 ** zoom))), int(round(optimal_zoom_h * hh / (0.5 ** zoom)))),
                Image.ANTIALIAS)
...
def try_to_save():
    ...
    for label in OPTION_LABELS:
        ...
        if "FIELD" in OPTION_NAMES[OPTION_LABELS.index(label)][0]:
            ET.SubElement(myroot, true_label).text = str(
                string_vars[len(OPTION_LABELS) - 1 - OPTION_LABELS.index(label)].get())
        else:
...
def ask():  # Open image file and see if xml exists
    ...
    if os.path.isfile(xml_path):
        ...
        m = tkMessageBox.showwarning(
            message="Associated annotations already exist for this image, further modifications will overwrite the file.")
...
def resize(s):
    global zoom, resize_once, x_image, y_image, canvas_s, optimal_zoom_w, optimal_zoom_h, zoom
    resize_once = False
    if s == "zoomin":
        zoom += 1
    if s == "zoomout":
